...
public class DatabaseService {
	...
	private static final String UNDIRECTED_SIMILARITY_QUERY = "SELECT year, association AS value FROM %s WHERE (word1=:word1 AND word2=:word2) OR (word1=:word2 AND word2=:word1) ORDER BY year ASC";
	...
	public DatabaseService(Sql2o sql2o, String path) throws DatabaseUnitException, Exception {
		this.sql2o = sql2o;
		readDemo(path);
		initializeMapping();
	}
	...
	private void initializeMapping() {
		...
		try (Connection con = sql2o.open()) {
			for (WordAndID wordAndId : con.createQuery(sql).executeAndFetch(WordAndID.class)) {
	...
	public void getTables() {
		...
		try (Connection con = sql2o.open()) {
			List<String> mostSimilar = con.createQuery(sql).executeScalarList(String.class);
			System.out.println(mostSimilar);
		}
	}

	public List<YearAndValue> getYearAndAssociation(String table, boolean directed, String word1, String word2)
			throws Exception {
		return getYearAndAssociation(table, directed, word2IdMapping.get(word1), word2IdMapping.get(word2));
	}

	List<YearAndValue> getYearAndAssociation(String table, boolean directed, int word1Id, int word2Id)
			throws Exception {
		String sql = directed ? String.format(DIRECTED_SIMILARITY_QUERY, table)
				: String.format(UNDIRECTED_SIMILARITY_QUERY, table);
		try (Connection con = sql2o.open()) {
			List<YearAndValue> mostSimilar = con.createQuery(sql).addParameter("word1", word1Id)
					.addParameter("word2", word2Id).executeAndFetch(YearAndValue.class);
			return mostSimilar;
		}
	}

	public List<YearAndValue> getYearAndFrequencyn(String table, String word) throws Exception {
		return getYearAndFrequencyn(table, word2IdMapping.get(word));
	}

	List<YearAndValue> getYearAndFrequencyn(String table, int wordId) throws Exception {
		try (Connection con = sql2o.open()) {
			return con.createQuery(String.format(FREQUENCY_QUERY, table)).addParameter("word", wordId)
	...
	public List<Integer> getYears(String word) throws Exception {
		...
		try (Connection con = sql2o.open()) {
			return con.createQuery(sql).addParameter("word", wordId).executeScalarList(Integer.class);
		}
	}

	public List<String> getMostSimilarWordsInYear(String givenWord, Integer year, int limit) {
		int givenWordId = word2IdMapping.get(givenWord);
		List<String> words = new ArrayList<>();
		String sql = String.format(MOST_SIMILAR_QUERY, TEST_SIMILARITY);
		try (Connection con = sql2o.open()) {
			for (IDAndID wordId : con.createQuery(sql).addParameter("givenWord", givenWordId).addParameter("year", year)
					.addParameter("limit", limit).executeAndFetch(IDAndID.class)) {
	...
	public List<String> getTopContextWordsInYear(String table, String givenWord, Integer year, int limit) {
		int givenWordId = word2IdMapping.get(givenWord);
		List<String> words = new ArrayList<>();
		String sql = String.format(TOP_CONTEXT_QUERY, table);
		try (Connection con = sql2o.open()) {
			for (Integer wordId : con.createQuery(sql).addParameter("givenWord", givenWordId).addParameter("year", year)
					.addParameter("limit", limit).executeScalarList(Integer.class)) {
	...
	void readDemo(String path) throws Exception {
		try (Connection con = sql2o.open()) {
			...
			con.createQuery("CREATE TABLE " + TEST_WORDIDS + " (word TEXT, id INTEGER, PRIMARY KEY(word,id) );")
					.executeUpdate();
			String assocTable = " (word1 INTEGER, word2 INTEGER, year SMALLINT, association REAL, PRIMARY KEY(word1, word2, year) );";
			con.createQuery("CREATE TABLE " + TEST_SIMILARITY + assocTable).executeUpdate();
			con.createQuery("CREATE TABLE " + TEST_PPMI + assocTable).executeUpdate();
			con.createQuery("CREATE TABLE " + TEST_FREQUENCY
					+ " (word INTEGER, year SMALLINT, frequency REAL, PRIMARY KEY(word, year));").executeUpdate();
		}

		// ID mapping
		try (Connection con = sql2o.beginTransaction()) {
			org.sql2o.Query query = con.createQuery("INSERT INTO " + TEST_WORDIDS + " (word, id) VALUES (:word, :id)");
			for (String[] x : Files.lines(Paths.get(path + WORDIDS_CSV)).map(x -> x.split(","))
					.collect(Collectors.toList()))
				query.addParameter("word", x[0]).addParameter("id", x[1]).addToBatch();
	...
	private void importFrequency(String tableName, String fileName) throws IOException {
		try (Connection con = sql2o.beginTransaction()) {
			org.sql2o.Query query = con.createQuery(
					"INSERT INTO " + tableName + " (word, year, frequency) VALUES (:word, :year, :frequency)");
			for (String[] x : Files.lines(Paths.get(fileName)).map(x -> x.split(",")).collect(Collectors.toList()))
				query.addParameter("word", x[0]).addParameter("year", x[1]).addParameter("frequency", x[2])
						.addToBatch();
			query.executeBatch(); // executes entire batch
			con.commit(); // remember to call commit(), else sql2o will
							// automatically rollback.
	...
	private void importAssociation(String tableName, String fileName) throws IOException {
		try (Connection con = sql2o.beginTransaction()) {
			...
			for (String[] x : Files.lines(Paths.get(fileName)).map(x -> x.split(",")).collect(Collectors.toList())) {
				query.addParameter("word1", x[0]).addParameter("word2", x[1]).addParameter("year", x[2])
